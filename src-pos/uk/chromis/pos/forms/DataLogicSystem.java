...
public class DataLogicSystem extends BeanFactoryDataSingle {
    ...
    protected SentenceFind m_adminPin;
    protected SentenceFind m_timeadminPin;

    private String SQL;
    private Map<String, byte[]> resourcescache;
    private Session s;
    
    private TseInfoCryptovision m_Tse;
    private Boolean m_TseActive = false;
    ...
    public void init(Session s) {
     ...
     //   m_sInitScript = "/uk/chromis/pos/scripts/" + s.DB.getName();
        ...
        m_adminPin = new StaticSentence(s, "SELECT TSE_ADMINPIN FROM SITES WHERE GUID = (SELECT GUID FROM SITEGUID)", SerializerWriteString.INSTANCE, SerializerReadString.INSTANCE);
        m_timeadminPin = new StaticSentence(s, "SELECT TSE_TIMEADMINPIN FROM SITES WHERE GUID = (SELECT GUID FROM SITEGUID)", SerializerWriteString.INSTANCE, SerializerReadString.INSTANCE);
...
//*******************************************************************        
        ...
        m_insertcash = new StaticSentence(s, "INSERT INTO CLOSEDCASH(MONEY, HOST, HOSTSEQUENCE, DATESTART, DATEEND, MONEYSTART, MONEYEND, MONEYDIFFERENCE, TERMINALSERIAL) "
...
// added 17.12.2016 update import profile         
    ...
    public final String getTerminalSerial() {
        // 00 - 36: = 0438bcba-8c58-4ff6-b127-af69fe3fe354  => Achtung! zu lang für mapERStoKey (max. 30 Zeichen) !!!
        // 09 - 36: = 8c58-4ff6-b127-af69fe3fe354
        try {
            return m_siteGuid.list().get(0).toString().substring(9, 36);
        } catch (BasicException ex) {
            return "";
        }
    }
    ...
    public final String getTseAdminPin() throws BasicException {
        return (String) m_adminPin.find();
    }

    public final String getTseTimeadminPin() throws BasicException {
        return (String) m_timeadminPin.find();
    }
