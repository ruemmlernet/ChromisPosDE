...
public class DataLogicSales extends BeanFactoryDataSingle {
    ...
    public final SentenceList getTaxList(String siteGuid) {
        return new StaticSentence(s, "SELECT "
                ...
                + "TAXES.RATECASCADE, "
                + "TAXES.RATEORDER, "
                + "TAXCATEGORIES.TSE_TAXCAT, "
                + "TAXES.SITEGUID "
                + "FROM TAXES, TAXCATEGORIES "
                + "WHERE TAXES.CATEGORY = TAXCATEGORIES.ID "
                + "AND TAXES.SITEGUID = '"
                + siteGuid
                + "' ORDER BY TAXES.NAME", null, new SerializerRead() {
            @Override
            public Object readValues(DataRead dr) throws BasicException {
                return new TaxInfo(
                        dr.getString(1),
    ...
    public final TicketInfo loadTicket(final int tickettype, final int ticketid) throws BasicException {
        ...
        if (ticket != null) {
            ...
            ticket.setLines(new PreparedSentence(s, "SELECT L.TICKET, L.LINE, L.PRODUCT, L.ATTRIBUTESETINSTANCE_ID, L.UNITS, L.PRICE, T.ID, T.NAME, T.CATEGORY, T.CUSTCATEGORY, T.PARENTID, T.RATE, T.RATECASCADE, T.RATEORDER, TC.TSE_TAXCAT, L.ATTRIBUTES, L.REFUNDQTY "
                    + "FROM TICKETLINES L, TAXES T, TAXCATEGORIES TC WHERE L.TAXID = T.ID AND T.CATEGORY = TC.ID AND L.TICKET = ? ORDER BY L.LINE", SerializerWriteString.INSTANCE, new SerializerReadClass(TicketLineInfo.class
                    )).list(ticket.getId()));
            ticket.setPayments(new PreparedSentence(s, "SELECT PAYMENT, TOTAL, TRANSID, TENDERED, CARDNAME FROM PAYMENTS WHERE RECEIPT = ? ", SerializerWriteString.INSTANCE, new SerializerReadClass(PaymentInfoTicket.class
                    )).list(ticket.getId()));
            // TSE-Daten laden
            Object[] record = (Object[]) new StaticSentence(s, "SELECT IFNULL(R.TSE_LOGTIME_START,0), "
                + "IFNULL(R.TSE_LOGTIME_END,0), "
                + "IFNULL(C.TERMINALSERIAL,''), "
                + "IFNULL(R.TSE_SIGNATURECOUNTER,0), "
                + "IFNULL(R.TSE_SIGNATUREVALUE,''), "
                + "IFNULL(R.TSE_TRANSACTIONNUMBER,0), "
                + "IFNULL(R.TSE_TIMEFORMAT,''), "
                + "IFNULL(R.TSE_HASHALGORYTHMUS,''), "
                + "IFNULL(R.TSE_STATUS,''), " 
                + "IFNULL(R.TSE_PUBLICKEY,'') FROM RECEIPTS R LEFT JOIN CLOSEDCASH C ON R.MONEY = C.MONEY WHERE ID = ?", SerializerWriteString.INSTANCE, 
                    new SerializerReadBasic(new Datas[]{Datas.DOUBLE, Datas.DOUBLE, Datas.STRING, Datas.DOUBLE, Datas.STRING, Datas.DOUBLE, 
                        Datas.STRING, Datas.STRING, Datas.STRING, Datas.STRING})).find(ticket.getId());
            Long x0 = Math.round( (Double)record[0]);
            Long x1 = Math.round( (Double)record[1]);
            Long x3 = Math.round( (Double)record[3]);
            Long x5 = Math.round( (Double)record[5]);
            ticket.setTseData(x0, x1, (String) record[2], x3, (String) record[4], x5, (String) record[6], (String) record[7], (String) record[8], (String) record[9]);
        }
        return ticket;
    }
            ...
            public Object transact() throws BasicException {
                ...
                new PreparedSentence(s, "INSERT INTO RECEIPTS (ID, MONEY, DATENEW, ATTRIBUTES, PERSON, TSE_LOGTIME_START, TSE_LOGTIME_END, TSE_SERIALNUMBER, TSE_SIGNATURECOUNTER, TSE_SIGNATUREVALUE, TSE_TRANSACTIONNUMBER, TSE_TIMEFORMAT, TSE_HASHALGORYTHMUS, TSE_STATUS, TSE_PUBLICKEY, GV_TYP, VORGANGSTYP) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", SerializerWriteParams.INSTANCE
                    ...
                    public void writeValues() throws BasicException {
                        ...
                        setInt(6, (int)ticket.getTseLogTimeStart());
                        setInt(7, (int)ticket.getTseLogTimeEnd());
                        setString(8, AppConfig.getTse().TseSerialNumber);
                        setInt(9, (int)ticket.getTseSignatureCounter());
                        setString(10, ticket.getTseSignatureValue());
                        setInt(11, (int)ticket.getTseTransactionNumber());
                        setString(12, ticket.getTseTimeFormat());
                        setString(13, ticket.getTseHashAlgorythmus());
                ...
                for (final PaymentInfo p : ticket.getPayments()) {
                    if (p.getName().equals("paperin")) {
                        payments.addPayment(p.getName(), p.getTotal(), p.getPaid(), p.getTransactionID());
                    } else {
                ...
                while (payments.getSize() >= 1) {
                    paymentinsert.exec(new DataParams() {
                        ...
                        public void writeValues() throws BasicException {
                            ...
                            if (pName.equals("paperin")) {
                                setString(5, getRetMsg);
                            } else {
                ...
                SentenceExec taxlinesinsert = new PreparedSentence(s, "INSERT INTO TAXLINES (ID, RECEIPT, TAXID, BASE, AMOUNT, TSE_TAXCAT)  VALUES (?, ?, ?, ?, ?, ?)", SerializerWriteParams.INSTANCE);
                    ...
                    for (final TicketTaxInfo tickettax : ticket.getTaxes()) {
                        taxlinesinsert.exec(new DataParams() {
                            ...
                            public void writeValues() throws BasicException {
                                ...
                                setInt(6, tickettax.getTaxInfo().getTseTaxCat());
                            }
                        });
                    }
                }
    ...
    public final void saveTicketTseFailed(final TicketInfo ticket) throws BasicException {
        // Wenn das Ticket abgebrochen wurde, den TSE-Vorgang dokumentieren !!!
        Transaction t;
        t = new Transaction(s) {
            @Override
            public Object transact() throws BasicException {
                ...
                new PreparedSentence(s, "INSERT INTO RECEIPTS (ID, MONEY, DATENEW, ATTRIBUTES, PERSON, TSE_LOGTIME_START, TSE_LOGTIME_END, TSE_SERIALNUMBER, TSE_SIGNATURECOUNTER, TSE_SIGNATUREVALUE, TSE_TRANSACTIONNUMBER, TSE_TIMEFORMAT, TSE_HASHALGORYTHMUS, TSE_STATUS, TSE_PUBLICKEY, GV_TYP, VORGANGSTYP) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", SerializerWriteParams.INSTANCE
                    ...
                    public void writeValues() throws BasicException {
                        ...
                        setInt(6, (int)ticket.getTseLogTimeStart());
                        setInt(7, (int)ticket.getTseLogTimeEnd());
                        setString(8, ticket.getTseSerialNumber());
                        setInt(9, (int)ticket.getTseSignatureCounter());
                        setString(10, ticket.getTseSignatureValue());
                        setInt(11, (int)ticket.getTseTransactionNumber());
                        setString(12, ticket.getTseTimeFormat());
                ...
                new PreparedSentence(s, "INSERT INTO PAYMENTS (ID, RECEIPT, PAYMENT, TOTAL, NOTES) VALUES (?, ?, ?, ?, ?)", SerializerWriteParams.INSTANCE
                ).exec(new DataParams() {
                    @Override
                    public void writeValues() throws BasicException {
                        setString(1, UUID.randomUUID().toString());
    ...
    * */
    ...
    // in Deutschland muss die BON_ID / Beleg-Nr. / Ticket-Nr. über ALLE VORGÄNGE eindeutig sein !!!!
    ...
    public final SentenceExec getPaymentMovementInsert() {
        return new SentenceExecTransaction(s) {
            ...
            public int execInTransaction(Object params) throws BasicException {
                ...
                Object[] params2 = new Object[20];    
                Object[] payment = (Object[]) params;
                for (int ix=0; ix<=7; ix++) params2[ix] = payment[ix];
                // User:
                params2[18] = payment[8];
                params2[19] = getNextTicketIndex();
                // TSE ist aktiv und (hoffentlich) richtig konfiguriert !!!
                if (mitTse) {

                    TseProcessData.clear();
                    TseProcessData.bruttoNull = (Double) payment[5];
                    TseProcessData.vorgangsTyp = "Beleg";
                    TseProcessData.zahlungBarEUR = (Double) payment[5];
                    TSE.StartTransactionResult str = AppConfig.getTse().startTransaction(AppConfig.getTerminalSerial(), TseProcessData.getProcessData(), AppConfig.getTse().getProcesstypeKassenbeleg(), "");
                    TSE.FinishTransactionResult ftr = null;
                    if (str != null) {
                        ftr = AppConfig.getTse().finishTransaction(AppConfig.getTerminalSerial(), str.transactionNumber, TseProcessData.getProcessData(), AppConfig.getTse().getProcesstypeKassenbeleg(), "");
                    }
                    // 7 = GV_TYP
                    if (ftr != null) {
                        params2[8]  = Double.parseDouble(Long.toString(str.logTime));
                        params2[9]  = Double.parseDouble(Long.toString(ftr.logTime));
                        params2[10] = Hex.toHexString(ftr.serialNumber);
                        params2[11] = Double.parseDouble(Long.toString(ftr.signatureCounter));
                        params2[12] = Hex.toHexString(ftr.signatureValue);
                        params2[13] = Double.parseDouble(Long.toString(str.transactionNumber));
                        params2[14] = AppConfig.getTse().getTimeFormat();
                        params2[15] = AppConfig.getTse().getSignaturAlgorithmus();
                        params2[16] = AppConfig.getTse().getCertificationId();
                        params2[17] = AppConfig.getTse().getPublicKey();
                    } else {
                        params2[8]  = System.currentTimeMillis()/1000;
                        params2[9]  = System.currentTimeMillis()/1000;
                        params2[10] = "";
                        params2[11] = 0.0; 
                        params2[12] = "";
                        params2[13] = 0.0; 
                ...
                new PreparedSentence(s, "INSERT INTO TICKETS (ID, TICKETTYPE, TICKETID, PERSON, STATUS) VALUES (?, 3, ?, ?, 0)", 
                                     new SerializerWriteBasicExt(paymenttabledatas, new int[]{0, 19, 18})).exec(params2);
